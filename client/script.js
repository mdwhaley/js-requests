...
// PROBLEM 1
/*
    In the index.html file in this folder there is a button with 
    an id of 'say-hello-button'!
...
// PROBLEM 3
/*
    Now you can see that the button colors change, but they do not 
    change back when we take the mouse off of the button.

    Write another function that changes the button back to its 
...
/*
    ...
    Use axios inside the ohMy function to make a GET request 
    to 'http://localhost:3000/animals' 
    
    Handle the promise that's returned with a .then, which you should 
    pass a callback function to. Inside the callback function, 
...
const ohMy = () => {
  ...
  axios.get("http://localhost:3000/animals").then((res) => {
    for (i = 0; i < res.data.length; i++) {
      const ohMyPara = document.createElement("p");
      const ohMyText = document.createTextNode(res.data[i]);
      ohMyPara.appendChild(ohMyText);
      console.log(ohMyPara);
      document.body.appendChild(ohMyPara);
    }
  });
};
...
/*
    ...
    take in a callback -- the callback function should print the response.data.
...
const repeatMyParam = () => {
  //YOUR CODE HERE
  axios
    .get("http://localhost:3000/repeat/What say what one more time")
    .then((res) => {
...
const getRequest = () => {
  axios
    .get("http://localhost:3000/repeat/What's up bro-sep-a-rito")
    .then((res) => {
      let sendAQuery = document.getElementById("query-button");
      sendAQuery.textContent = res.data;
    });
};
...
// PROBLEM 9
/* 
    Back in the ohMy function on Problem 5, replace the console log in 
    the promise's callback with a for loop that loops over res.data. 
